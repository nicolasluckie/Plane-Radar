...
class RadarDisplay:
    ...
    def init_palette(self):
        ...
        theme.COLOR_BACKGROUND = self.display.color565(
            theme.BG_R, theme.BG_G, theme.BG_B)
        theme.COLOR_GRID = self.display.color565(
            theme.GRID_R, theme.GRID_G, theme.GRID_B)
        ...
        theme.COLOR_RUNWAY = self.display.color565(
            theme.RUNWAY_R, theme.RUNWAY_G, theme.RUNWAY_B)
    ...
    def speed_line_length_px(self, gs_knots):
        ...
        px = (gs_knots * km_per_knot_per_horizon * theme.GRID_OUTER_RADIUS /
              theme.AIRCRAFT_TRACK_REF_OUTER_KM * theme.AIRCRAFT_TRACK_LENGTH_SCALE)
    ...
    def draw_heading_triangle(self, cx, cy, heading_deg, color):
        ...
        self.display.triangle(tip_x, tip_y, 
    ...
    def draw_speed_vector(self, cx, cy, heading_deg, track_deg, gs_knots, color):
        ...
        self.display.line(tip_x, tip_y, ex, ey, color, 
                         width=int(theme.AIRCRAFT_TRACK_LINE_HALF_WIDTH * 2))
    ...
    def draw_cardinal_labels(self):
        ...
        self.display.text("N", cx - text_width // 2 + 1, theme.CARDINAL_NORTH_OFFSET_Y, theme.COLOR_LABEL, font=font)
        
        # S (centered horizontally at bottom, bumped right 1px)
        bbox = font.getbbox("S")
        text_width = bbox[2] - bbox[0]
        self.display.text("S", cx - text_width // 2 + 1, edge + theme.CARDINAL_SOUTH_OFFSET_Y - 14, theme.COLOR_LABEL, font=font)
        ...
        text_width = bbox[2] - bbox[0]
        text_height = bbox[3] - bbox[1]
        self.display.text("E", edge - text_width - 3, cy - text_height // 2 - 2, theme.COLOR_LABEL, font=font)
    
    def draw_scale_label(self, cx, cy, outer_radius):
        """Draw scale label with black background."""
        label = range_manager.format_current_ring3_label()
        ...
        self.display.rect(box_x, box_y, box_width, box_height,
                         theme.COLOR_BACKGROUND, fill=theme.COLOR_BACKGROUND)
    ...
    def draw_beyond_ring_dot(self, x, y):
        ...
        self.display.fill_circle(x, y, theme.BEYOND_RING_DOT_RADIUS_PX, 
                                theme.COLOR_AIRCRAFT)
    ...
    def _get_text_bbox(self, text, font):
    ...
    def draw_aircraft_tag(self, x, y, aircraft, existing_tags=None):
        ...
        if aircraft.type:
            self.display.text(aircraft.type, tag_x, tag_y + 12, theme.COLOR_TAG_TYPE, font=font_regular)
        
        if aircraft.alt:
            self.display.text(aircraft.alt, tag_x, tag_y + 24, theme.COLOR_TAG_ALTITUDE, font=font_bold)
        
        if aircraft.squawk:
            self.display.text(aircraft.squawk, tag_x, tag_y + 36, theme.COLOR_TAG_SQUAWK, font=font_bold)
    ...
    def draw_aircraft(self, aircraft_list):
        ...
        for ac, x, y, _ in inside_items:
            self.draw_speed_vector(x, y, ac.nose_deg, ac.track_deg, 
                                  ac.gs_knots, theme.COLOR_TRACK_VECTOR)
