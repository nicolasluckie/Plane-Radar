...
try:
    ...
    logger.warning(
        "Could not check /dev/fb1. Enable dtoverlay=gc9a01 in /boot/config.txt and reboot."
    )
...
class DisplayDriver:
    ...
    def init(self):
        ...
        try:
            self.font = ImageFont.truetype(
                "/usr/share/fonts/truetype/noto/NotoSans-Regular.ttf", 10
            )
            self.font_bold = ImageFont.truetype(
                "/usr/share/fonts/truetype/noto/NotoSans-Bold.ttf", 10
            )
        except OSError:
            try:
                self.font = ImageFont.truetype(
        ...
        try:
            ...
            logger.info(
                "Framebuffer initialized: %dx%d at %s", self.width, self.height, self.fb_path
            )
    ...
    def circle(self, x, y, radius, color, width=1):
        ...
        rgb = self._rgb565_to_rgb(color)
        self.draw.ellipse(
            (x - radius, y - radius, x + radius, y + radius), outline=rgb, width=width
        )
